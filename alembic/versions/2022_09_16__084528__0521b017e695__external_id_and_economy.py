...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_coins_economy_record_modified'), 'coins_economy', ['record_modified'], unique=False)

    op.alter_column("user", 'cognito_user_id', nullable=False, server_default="COGNITO", new_column_name="external_id")
    op.add_column('user', sa.Column('external_id_type', sa.Text(), server_default='COGNITO', nullable=False))
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('user', sa.Column('cognito_user_id', sa.VARCHAR(), autoincrement=False, nullable=False))
    op.alter_column("user", 'external_id', nullable=False, new_column_name="cognito_user_id")
